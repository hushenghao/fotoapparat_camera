package com.dede.default_camera.view;
...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, int size) {
        ...
        button_state = CameraDefaultActivity.BUTTON_STATUS_ALL;  //初始化按钮为可录制可拍照
        duration = 30 * 1000;              //默认最长录制时间为30s
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                //判断按钮状态是否为可录制状态
                if ((button_state == CameraDefaultActivity.BUTTON_STATUS_ONLY_RECORDER || button_state == CameraDefaultActivity.BUTTON_STATUS_ALL))
                    postDelayed(longPressRunnable, 300);    //同时延长500启动长按后处理的逻辑Runnable
                break;
            case MotionEvent.ACTION_MOVE:
                if (captureLisenter != null
                        && state == STATE_RECORDERING
                        && (button_state == CameraDefaultActivity.BUTTON_STATUS_ONLY_RECORDER || button_state == CameraDefaultActivity.BUTTON_STATUS_ALL)) {
    ...
    private void handlerUnpressByState() {
        ...
        switch (state) {
            //当前是点击按下
            case STATE_PRESS:
                if (captureLisenter != null && (button_state == CameraDefaultActivity.BUTTON_STATUS_ONLY_CAPTURE || button_state ==
                        CameraDefaultActivity.BUTTON_STATUS_ALL)) {
